...
def show_products():
    ...
    for number, product in enumerate(products, 1):
        print(f"{number}. {product.name} | Категорія: {product.category} | Ціна: {product.price} | Залишок: {product.quantity}")
...
def add_product_to_order():
    ...
    while True:
        try:
            target_product = get_current_product("Which product you want to add to order?: ")
...
def add_new_order():
    ...
    for number, order in enumerate(current_customer.orders, 1):
        ...
        print(f"Order #{number}: Number of products: {len(order.order_list)} | Total cost: {actual_total} ")
...
def change_price():
    ...
    while True:
        try:
            target_product = get_current_product("For which product you want to change price?: ")
            while True:
                try:
...
def change_quantity():
    ...
    while True:
        try:
           target_product = get_current_product("For which product you want to change quantity?: ")  
           while True:
                try:
