...
def mock_warehouse_service():
    with patch("bookshop.services.WarehouseClient.reserve_book") as mock_reserve, patch(
        "bookshop.services.WarehouseClient.confirm_sale"
    ) as mock_confirm, patch(
        "bookshop.services.WarehouseClient.release_reservation"
    ) as mock_release, patch(
        "bookshop.services.WarehouseClient._get_auth_headers"
    ) as mock_auth:
...
def test_flow_order_total_price_calculation():
    ...
    client.post(
        reverse("add_to_cart", kwargs={"book_id": book1.id}), data={"quantity": 2}
    )
    client.post(
        reverse("add_to_cart", kwargs={"book_id": book2.id}), data={"quantity": 1}
    )
...
def test_flow_cart_cleared_after_order_submission():
    ...
    client.post(
        reverse("add_to_cart", kwargs={"book_id": book.id}), data={"quantity": 1}
    )
    client.post(reverse("submit_cart"))
